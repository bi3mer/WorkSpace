...
def safeBoundary(matrix, x, y):
	if x > xBoundaryMin and x < len(matrix) - 1 and y != yBoundaryMin and y < len(matrix):
		return True
	return False

def countAliveGrounds(procMap, x, y):
	count = 0
	if safeBoundary(procMap, x - 1, y) and procMap[y][x - 1] == ground:
		count += 1
	if safeBoundary(procMap, x , y + 1) and procMap[y + 1][x] == ground:
		count += 1
	if safeBoundary(procMap, x , y - 1) and procMap[y - 1][x] == ground:
		count += 1
	if safeBoundary(procMap, x + 1, y) and procMap[y][x + 1] == ground:
...
def countAliveWalls(procMap, x, y):
	count = 0
	if safeBoundary(procMap, x - 1, y) and procMap[y][x - 1] == wall:
		count += 1
	if safeBoundary(procMap, x , y + 1) and procMap[y + 1][x] == wall:
		count += 1
	if safeBoundary(procMap, x , y - 1) and procMap[y - 1][x] == wall:
		count += 1
	if safeBoundary(procMap, x + 1, y) and procMap[y][x + 1] == wall:
...
for i in range(xBoundaryMax):
	for j in range(yBoundaryMax):
		if safeBoundary(procMap,i, j) and random.random() > chanceToLive:
			procMap[i][j] = ground
